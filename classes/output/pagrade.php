 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pagrade /* MoodleQuickForm_pagrade */ extends \MoodleQuickForm_group{
     ...
     */
    public function MoodleQuickForm_pagrade($elementname = null, $elementlabel = null, $options = array(), $attributes = null) {
    ...
    public function _createElements() {
        ...
        $langscale = get_string('pagradetypescale', 'mod_peerassessment');
        $scaleselect = $this->createFormElement('select', 'pagrade_scale', $langscale, $scales, $attributes);
//        @\MoodleQuickForm::createElement('select', 'pagrade_scale', $langscale, $scales, $attributes);
        $scaleselect->setHiddenLabel = false;
        $scaleselectid = $this->generate_pagrade_subelement_id('pagrade_scale');
        $scaleselect->updateAttributes(array('id' => $scaleselectid));

        // Maximum grade textbox.
        $langmaxgrade = get_string('pagrademaxgrade', 'mod_peerassessment');
        $maxgrade = $this->createFormElement('text', 'pagrade_point', $langmaxgrade, array());
//@\MoodleQuickForm::createElement('text', 'pagrade_point', $langmaxgrade, array());
        $maxgrade->setHiddenLabel = false;
        $maxgradeid = $this->generate_pagrade_subelement_id('pagrade_point');
        $maxgrade->updateAttributes(array('id' => $maxgradeid));

        // Grade type select box.
        $gradetype = array(
                //'none' => get_string('pagradetypenone', 'mod_peerassessment'),
                'scale' => get_string('pagradetypescale', 'mod_peerassessment'),
                'point' => get_string('pagradetypepoint', 'mod_peerassessment'),
        );
        $langtype = get_string('pagradetype', 'mod_peerassessment');
        $typeselect = $this->createFormElement('select', 'pagrade_type', $langtype, $gradetype, $attributes, true);
        $typeselect->setHiddenLabel = false;
        $typeselectid = $this->generate_pagrade_subelement_id('pagrade_type');
    ...
    public function exportValue(&$submitvalues, $notused = false) {
        ...
        $type = (isset($vals['pagrade_type'])) ? $vals['pagrade_type'] : 'none';
        $point = (isset($vals['pagrade_point'])) ? $vals['pagrade_point'] : null;
        $scale = (isset($vals['pagrade_scale'])) ? $vals['pagrade_scale'] : null;
    ...
    public function onQuickFormEvent($event, $arg, &$caller) {
        switch ($event) {
            case 'createElement':
                ...
                // Set disable actions.
                $caller->disabledIf($name.'[pagrade_scale]', $name.'[pagrade_type]', 'neq', 'scale');
                $caller->disabledIf($name.'[pagrade_point]', $name.'[pagrade_type]', 'neq', 'point');
                ...
                $checkmaxgrade = function($val) {
                    ...
                    if (isset($val['pagrade_type']) && $val['pagrade_type'] === 'point') {
                        if (!isset($val['pagrade_point'])) {
                            return false;
                        }
                        return $this->validate_point($val['pagrade_point']);
                    }
                    return true;
                };
                $checkvalidscale = function($val) {
                    // Closure to validate a scale value. See the note above about scope if this confuses you.
                    if (isset($val['pagrade_type']) && $val['pagrade_type'] === 'scale') {
                        if (!isset($val['pagrade_scale'])) {
                            return false;
                        }
                        return $this->validate_scale($val['pagrade_scale']);
                    }
                    return true;
                };
    ...
    /**
     * Generates the id attribute for the subelement of the pagrade group.
     *
     * Uses algorithm similar to what {@link HTML_QuickForm_element::_generateId()}
     * does but takes the name of the wrapping pagrade group into account.
     *
     * @param string $subname the name of the HTML_QuickForm_element in this pagrade group
     * @return string
     */
    protected function generate_pagrade_subelement_id($subname) {
